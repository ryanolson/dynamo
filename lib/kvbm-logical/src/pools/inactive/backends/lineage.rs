...
impl<T: BlockMetadata> LineageNode<T> {
    fn new(block: Block<T, Registered>, lineage_hash: PositionalSequenceHash, tick: u64) -> Self {
...
impl<T: BlockMetadata> LineageBackend<T> {
    ...
    pub fn remove(&mut self, lineage_hash: &PositionalSequenceHash) -> Option<Block<T, Registered>> {
        let position = lineage_hash.position();
        let fragment = lineage_hash.current_hash_fragment();
...
impl<T: BlockMetadata> InactivePoolBackend<T> for LineageBackend<T> {
    ...
    fn scan_matches(
        ...
        hashes: &[PositionalSequenceHash],
        _touch: bool,
    ) -> Vec<(PositionalSequenceHash, Block<T, Registered>)> {
    ...
    fn has_block(&self, seq_hash: PositionalSequenceHash) -> bool {
...
mod tests {
    ...
    fn test_collision_would_be_detected() {
        ...
        // the same position+fragment) without constructing invalid PositionalSequenceHash
        ...
        // without bypassing PositionalSequenceHash invariants, this test serves as
        // documentation.
    }
}
