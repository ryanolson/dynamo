...
pub struct BlockManagerConfigBuilder<T: BlockMetadata> {
    ...
    /// Optional custom block allocator (defaults to DequeBlockAllocator)
...
impl<T: BlockMetadata> BlockManagerConfigBuilder<T> {
    ...
    /// Use a custom [`BlockAllocator`] for the reset pool.
    ///
    /// When not set, a [`DequeBlockAllocator`](crate::pools::DequeBlockAllocator)
    /// (simple FIFO queue) is used by default.
    ///
    /// Custom allocators can perform arbitrary work when blocks are allocated
    /// or returned — for example, communicating with a remote block server.
