...
//!
//! [`Reset`] is public so that external [`BlockAllocator`](crate::pools::BlockAllocator)
//! implementations can work with `Block<T, Reset>`. [`Staged`] and
//! [`Registered`] are crate-internal -- callers interact with them only
//! through the RAII guard types.
//!
...
pub(crate) struct Registered {
    pub(crate) sequence_hash: SequenceHash,
    pub(crate) registration_handle: BlockRegistrationHandle,
}
