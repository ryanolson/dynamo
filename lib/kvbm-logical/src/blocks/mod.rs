...
///
/// Blocks progress through a fixed state machine (`Reset` → `Staged` →
/// `Registered`) enforced at the type level. The RAII guard types
/// ([`MutableBlock`], [`CompleteBlock`], [`ImmutableBlock`]) wrap a `Block`
/// and handle automatic return to the allocator on drop.
///
/// Custom [`BlockAllocator`](crate::pools::BlockAllocator) implementations
/// create `Block<T, Reset>` via [`Block::new`] and receive them back via
/// [`BlockAllocator::insert`](crate::pools::BlockAllocator::insert).
#[derive(Debug)]
